...
/* Check for calibration failure */} while (ADC0->SC3 & ADC_SC3_CALF_MASK);
/* Compute and store plus-side calibration value */
ADC0->PG = (((Int16) ADC0->CLPS + (Int16) ADC0->CLP4 +(Int16) ADC0->CLP3 + (Int16) ADC0->CLP2 +(Int16) ADC0->CLP1 + (Int16) ADC0->CLP0) >> 1) | (Int16) 0x8000;
...
int main (void) {
  ...
  /* Put main program loop code here */
	PutStringSB("Type servo position ",0);
	PutStringSB("number (1-5): ",0);
	servenum =	GetChar();
	if(servenum <= '5' && servenum >='1'){
	PutChar(servenum);
	newline();
		
		
	index = (UInt8) servenum - '1';
	tableval =  DAC0_table [index];
		
	seg = servenum - '0';
	DAC0->DAT[0].DATL = (UInt8) (DAC0_table [seg - 1] & 0xFF);
	DAC0->DAT[0].DATH = (UInt8) (DAC0_table [seg - 1] >> 8);
	PutStringSB("          Original digital value: ",0);
	PutStringSB("0x",0);
	...
	newvalue = ADC0->R[0];
	PutStringSB("               New digital value: 0x",0);

	PutNumHex(newvalue - 1);
	newline();
	newvalue = ((newvalue* 5) /1024 ) + 1 ;
	PutStringSB("               'Analog' ",0);
	PutStringSB("position: ",0);
	PutNumUB(newvalue);
	TPM0->CONTROLS[2].CnV = PWM_duty_table [seg];
	newline();
	}
  } /* do forever */
